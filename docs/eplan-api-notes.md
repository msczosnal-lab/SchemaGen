...
## Sesja 1.7g cd. — 2026-06-13 (Plan B: globalny FUNC_COUNTER — ZW)

- **Werdykt CONFIGSCHEME (Plan A): NIE działa dla MA.** Test Filipa po deployu dual-pass: silniki +B2/+B4 nadal `-MA1`/`-MA1`. FC zmieniają się poprawnie (ta sama lokalizacja obiektowa, różne strony). Natywny `/CONFIGSCHEME` nie wymusił globalnego licznika dla DT rozłożonych po różnych lokalizacjach. `configScheme` dla reguły MA wyczyszczony w `numbering-rules.xml`.
- **Plan B (wdrożony): wymuszenie `FUNC_COUNTER` w add-inie.** Nowa akcja `SchemaGenForceGlobalCounter` — zbiera funkcje główne (`IsMainFunction`) z `FUNC_CODE == IDENTIFIER` w kolejności stron SchemaGen i nadaje kolejne liczniki przez `func.NameParts` → `FUNC_COUNTER` (Transaction + SafetyPoint). Widoczne `DESIGNATION_PRODUCT` (`MA1`) jest **składane z FUNC_CODE + FUNC_COUNTER** — KB `datamodel.md:454`.
- **[WAŻNE] Dlaczego nie `<20010>`:** bezpośredni zapis property name (`<20010>` / `func.Name`) to ślepa uliczka (S063113) — KB: name properties tylko przez `.NameParts`, a `DESIGNATION_PRODUCT` wyłącznie przez `FUNC_CODE`+`FUNC_COUNTER`. Zmiana licznika daje ten sam efekt (zmiana widocznego taga) ścieżką, którą EPLAN akceptuje.
- **Typ NameParts (potwierdzone buildem 1.7g):** getter `Function.NameParts` zwraca **`FunctionBasePropertyList`** (nie `FunctionPropertyList` — CS0266). Setter też przyjmuje `FunctionBasePropertyList`. `FUNC_COUNTER` jest składową tej klasy.
- **[RYZYKO] do potwierdzenia w EPLAN (Filip):** getter `Function.NameParts` musi zwracać obiekt z zapisywalnym `FUNC_COUNTER`, a setter `func.NameParts = parts` — utrzymać plant/lokalizację/kod. Jeśli wyjątek → trafia do `output/force-global-counter.json` (pole `log`, `changed < total`). Alternatywa gdy read-modify-write zawiedzie: budować świeży `FunctionBasePropertyList` z `DESIGNATION_PLANT`/`DESIGNATION_LOCATION`/`FUNC_CODE` + nowy `FUNC_COUNTER`.
- **Orkiestracja MVP:** pass 1 = renumber wszystkie reguły (FC bez regresji), pass 2 = `SchemaGenForceGlobalCounter` dla reguł `forceGlobalCounter="true"`. Audyt: `renumber-devices.json` (baseline) + `force-global-counter.json` (po wymuszeniu).
