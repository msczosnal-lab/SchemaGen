...
- **[RYZYKO] do potwierdzenia w EPLAN (Filip):** getter `Function.NameParts` (typ `FunctionPropertyList`) musi zwracać obiekt z zapisywalnym `FUNC_COUNTER`, a setter `func.NameParts = parts` — utrzymać plant/lokalizację/kod. Jeśli wyjątek → trafia do `output/force-global-counter.json` (pole `log`, `changed < total`). Alternatywa gdy read-modify-write zawiedzie: budować świeży `FunctionBasePropertyList` z `DESIGNATION_PLANT`/`DESIGNATION_LOCATION`/`FUNC_CODE` + nowy `FUNC_COUNTER`.
