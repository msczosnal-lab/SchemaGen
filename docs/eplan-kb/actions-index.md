...
| `edit` | Action class for edit functions: open a project, open a page with a name, open a page with a device name and open a page |
| `EplApiModuleAction` | Loads and registers an API Add-in. |
| `EsCorrectConnections` | Merges graphical properties (color, line type, layer...) of connection definition points into one signal definition poin |
...
| `partslist` | Action class for exporting and importing parts and other parts management items like addresses, constructions, terminals |
| `partsmanagementapi` | Action class for exporting and importing parts and other parts management items like addresses, constructions, terminals |
...
| `translate` | Action class for translate functions: translate a project, export missing translation list, and remove languages from a  |
...
| `XAMlExportProductionData2RASCenterAction` | Export of the construction spaces of the selected project in AutomationML format. The generated AutomationML file is int |
| `XAMlExportProductionData2SmartMountingAction` | Export of the construction spaces of the selected project in AutomationML format. The generated AutomationML file is int |
| `XCabCalculateEnclosureTotalWeightAction` | Calculates the total weight of a cabinet and writes it to the "Total weight" property (#36108 - FUNCTION3D_CABINET_TOTAL |
| `XCCreateGravingtextAction` | Generates an engraving text from the DTs of the source and target of the cable. By default, the designation is abbreviat |
...
| `XGedUpdateMacroAction` | Updates macros. It can be passed the full path of a project. When project is not opened, this action opens it and closes |
...
| `XPrjConvertBaseProjectsAction` | This action converts one ore more old basic projects (*.ept and *.epb files) to new basic projects (*.zw9). All basic pr |
...
## Akcje istotne dla SchemaGen

- **backup**: Action class for backup functions. Backs up a project and master data (forms, symbols, ...) to disk
- **check**: Action class for checking functions: check a project and check pages.
- **compress**: Action class to compress projects.
- **devicelist**: Action class for device list functions: import, export, and delete device lists.
- **edit**: Action class for edit functions: open a project, open a page with a name, open a page with a device name and open a page with name and set the cursor 
- **EsCorrectConnections**: Merges graphical properties (color, line type, layer...) of connection definition points into one signal definition point for each signal, if these gr
- **ExecuteScript**: Runs the given script.
- **export**: Action to export pages and projects in graphical, DXF, DWG, PXF format.
- **export3d**: Action to export installation spaces into 3d formats.
- **ExportNCData**: Action exports NC Data for machines.
...
- **MfExportRibbonBarAction**: Exports main ribbon bar customizing to XML file.
- **MfImportRibbonBarAction**: Imports main ribbon bar customizing from XML file.
- **partslist**: Action class for exporting and importing parts and other parts management items like addresses, constructions, terminals, accessory lists and accessor
- **partsmanagementapi**: Action class for exporting and importing parts and other parts management items like addresses, constructions, terminals, accessory lists and accessor
- **plcservice**: Exports/imports PLC data using the specified converter.
- **preparemacros**: Action for preparing project for macro generation.
- **print**: Action class to print projects and pages.
- **ProjectAction**: Runs an action upon a given project and closes project afterwards.
...
- **UpdateSegmentsFilling**: Calculates and sets value of property CABLINGSEGMENT_FILLING for all segments in project.
- **XAfActionSettingProject**: Sets the value of a project setting.
- **XAMlExportProductionData2RASCenterAction**: Export of the construction spaces of the selected project in AutomationML format. The generated AutomationML file is intended for import into the Ritt
- **XAMlExportProductionData2SmartMountingAction**: Export of the construction spaces of the selected project in AutomationML format. The generated AutomationML file is intended for import into the Ritt
- **XCMRemoveUnnecessaryNDPsAction**: Removes unnecessary net definition points of active project.
- **XCMUniteNetDefinitionPointsAction**: Unites net definition points placed on the same net in active project.
- **XEsGetProjectPropertyAction**: Gets a special property of the current project.
- **XEsSetProjectPropertyAction**: Sets a special property of a current project.
...
- **XPamsDeviceSelectionAction**: Selects device or updates device information. This object can be a project/function/connection.
- **XPrjActionUpgradeProjects**: This action upgrades one ore more projects to the actual database scheme version.
- **XPrjConvertBaseProjectsAction**: This action converts one ore more old basic projects (*.ept and *.epb files) to new basic projects (*.zw9). All basic projects in a folder are upgrade
- **XSDPreviewAction**: Opens or closes the preview of a project page or macro
- **XSettingsExport**: Exports settings to an XML file.
- **XSettingsImport**: Imports project-, station-, company- or user settings from an XML file.