"""Ad-hoc smoke GraphBuildera — liczby + nakladka per rola (po sicie/ROI/net-builderze).

Uruchom (WAZNE: venv z YOLO/OCR):
    .venv311\\Scripts\\python.exe smoke_graph.py *p040*
Wynik: data/output/calib/<strona>_graph.png + histogram rol + connections.
Kolory: wire=zielony, bus=niebieski, frame=szary, other=zolty, bbox symbolu=czerwony.
...
def main() -> None:
    ...
    for c in model.connections[:25]:
        pot = f" [{c.potential}]" if c.potential else ""
        print(f"  {c.from_ref} -> {c.to} ({c.kind}){pot}")
